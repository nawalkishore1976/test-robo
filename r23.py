...
class WayPoint:
    ...
    target_m1_pps: float = 0.0  # target left wheel pulses per second
    target_m2_pps: float = 0.0  # target right wheel pulses per second
    target_turn: float = 0.0    # target turn angle in degrees (should be small <5°)
    expected_total_turn: float = 0.0  # expected cumulative turn from start
    target_m1_steps: int = 0    # target cumulative left wheel steps
    target_m2_steps: int = 0    # target cumulative right wheel steps
...
class RobotPathPlannerV14:
    ...
    def calculate_differential_wheel_speeds(self, linear_speed: float, curvature: float) -> Tuple[float, float]:
        ...
        if curvature > 0:  # Left turn (counter-clockwise)
            left_radius = abs(radius) - WHEEL_BASE / 2
            right_radius = abs(radius) + WHEEL_BASE / 2
            left_speed = angular_velocity * left_radius
            right_speed = angular_velocity * right_radius
    ...
    def interpolate_circular_arc_fixed(self, start_anchor: AnchorPoint, end_anchor: AnchorPoint, start_time: float, 
        ...
        """FIXED: Interpolate points along circular arc with precomputed wheel speeds"""
        ...
        self.debug_print(f"  Curvature: {curvature:.4f} (1/radius)")
...
class RobotSimulationV14a:
    ...
    def simulate_robot_step_fixed(self, target_wp: WayPoint, dt: float) -> RobotState:
        ...
        right_speed += np.random.normal(0, 0.02)
    ...
    def animate_robot_motion(self):
        ...
        for i, target_wp in enumerate(self.planner.waypoints):
            ...
            print(f"W{i:2}: Target=({target_wp.x:5.1f},{target_wp.y:5.1f},{target_wp.get_heading_deg():6.1f}°) "
                  ...
                  f"M1/M2={self.robot_state.actual_m1_pps:.0f}/{self.robot_state.actual_m2_pps:.0f}pps "
                  f"Turn={self.robot_state.actual_turn:.1f}° TotalTurn={self.robot_state.actual_total_turn:.1f}° "
                  f"Steps={self.robot_state.actual_m1_steps}/{self.robot_state.actual_m2_steps}")
