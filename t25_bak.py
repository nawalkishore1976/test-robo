...
class RobotSimulationV14:
    ...
    def calculate_wheel_speeds_precomputed(self, target_wp: WayPoint) -> Tuple[float, float]:
        """Use precomputed wheel speeds with MINIMAL corrections"""
    
        # Start with precomputed target speeds (these are already correct!)
        target_left = target_wp.target_m1_pps / PULSES_PER_CM
        target_right = target_wp.target_m2_pps / PULSES_PER_CM
    
        # MINIMAL position-based correction only if far off course
        pos_error_x = target_wp.x - self.robot_state.x
        pos_error_y = target_wp.y - self.robot_state.y
        distance_error = math.sqrt(pos_error_x**2 + pos_error_y**2)
    
        # Only apply corrections if significantly off course
        if distance_error > 5.0:  # Only correct if >5cm off
            heading_error = target_wp.heading - self.robot_state.heading
            while heading_error > math.pi:
                heading_error -= 2 * math.pi
            while heading_error < -math.pi:
            ...
            correction = self.pid_heading.update(heading_error, 0.1) * 0.1  # Much smaller
        
            target_left -= correction
            target_right += correction
    
        # Limit speeds
        target_left = max(-MAX_WHEEL_SPEED, min(MAX_WHEEL_SPEED, target_left))
        target_right = max(-MAX_WHEEL_SPEED, min(MAX_WHEEL_SPEED, target_right))
    ...
    def simulate_robot_step_fixed(self, target_wp: WayPoint, dt: float) -> RobotState:
        ...
        # Get target wheel speeds (already correct from path planning)
        left_speed, right_speed = self.calculate_wheel_speeds_precomputed(target_wp)
        
        # Minimal noise
        left_speed += np.random.normal(0, 0.01)  # Reduced noise
        right_speed += np.random.normal(0, 0.01)
        
        # CORRECT differential drive kinematics
        linear_velocity = (left_speed + right_speed) / 2.0
        
        # FIXED: Angular velocity calculation
        # For North=0° system: positive angular_velocity = clockwise = right turn
        angular_velocity = (left_speed - right_speed) / WHEEL_BASE
        ...
        # FIXED: Position update for North=0° coordinate system
        # Use the AVERAGE heading during this time step for more accuracy
