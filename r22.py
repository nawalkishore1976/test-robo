...
class RobotSimulationV14a:
    ...
    def print_debug_comparison(self):
            ...
            print(f"{'Step':<4} {'Target Position & Heading':<35} {'Actual Position & Heading':<35} "
                f"{'Expected M1/M2':<15} {'Actual M1/M2':<15} {'Expected Turn':<12} {'Actual Turn':<12} {'Errors':<20}")
            print("-" * 150)
            for i, data in enumerate(self.debug_data):
                # Calculate expected values for this step
                target = data['target']
                actual = data['actual']
                ...
                print(f"{i:<4} Tgt=({target.x:5.1f},{target.y:5.1f},{target.get_heading_deg():5.1f}°) "
                      ...
                      f"ExpTrn={expected_turn:4.1f}° ActTrn={actual.actual_turn:4.1f}° "
                      f"Err: M1={m1_pps_error:+4.0f} M2={m2_pps_error:+4.0f} Trn={turn_error:+4.1f}°")
