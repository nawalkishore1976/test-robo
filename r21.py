...
# =============================================================================
# FIXED ROBOT PATH PLANNER V13 - NORTH = 0°
# =============================================================================
class RobotPathPlannerV13:
    ...
    def convert_commands_to_anchors_v13(self, commands: str) -> int:
        """V13: FIXED circular turn paths with North = 0°"""
        current_x, current_y = 0.0, 0.0
        current_dir = DIR_NORTH  # Start facing North (0°)
        self.anchors.clear()
        self.current_command = commands
        
        self.debug_print("=== V13: FIXED COMMANDS - NORTH = 0° ===")
    ...
    def interpolate_waypoints_v13(self) -> int:
        """Create waypoints including FIXED circular arc interpolation with North = 0°"""
        self.waypoints.clear()
        current_time = 0.0
        
        self.debug_print("=== V13: FIXED WAYPOINT INTERPOLATION - NORTH = 0° ===")
    ...
    def interpolate_circular_arc_fixed(self, start_anchor: AnchorPoint, end_anchor: AnchorPoint, start_time: float):
        """FIXED: Interpolate points along circular arc with North = 0°"""
        ...
        # Calculate the correct angle difference for exactly 90 degrees
        ...
        for j in range(num_points + 1):
            ...
            # For North=0° coordinate system: tangent = radius_angle + 90°
    ...
    def process_commands(self, commands: str):
        """Process commands with V13 fixes and North = 0°"""
        self.convert_commands_to_anchors_v13(commands)
        self.interpolate_waypoints_v13()

# =============================================================================
# FIXED ROBOT SIMULATION V13 - NORTH = 0°
# =============================================================================
class RobotSimulationV13:
    ...
    def calculate_wheel_speeds_fixed_old(self, target_wp: WayPoint) -> Tuple[float, float]:
        """FIXED: Calculate differential wheel speeds with proper control and North = 0°"""
        ...
        if distance_error > 0.1:  # Only calculate if there's significant distance
            desired_heading = math.atan2(pos_error_y, pos_error_x)
        else:
            desired_heading = target_wp.heading
        ...
        base_speed = min(target_wp.speed, distance_error * 3.0)  # Proportional to distance
        base_speed = max(MIN_WHEEL_SPEED, min(MAX_WHEEL_SPEED, base_speed))
        
        # Calculate turning component
        turn_speed = self.pid_heading.update(heading_error, 0.1) * 3.0  # Scale turning
    ...
    def calculate_wheel_speeds_fixed(self, target_wp: WayPoint) -> Tuple[float, float]:
        ...
        # Use the target waypoint's heading directly for smooth arc following
        desired_heading = target_wp.heading
        ...
        # Base speed - use target waypoint speed directly
        base_speed = target_wp.speed
        
        # For curves, reduce speed based on distance error to improve tracking
        if target_wp.curvature > 0:  # We're on a curve
            # Reduce speed if we're far from the target point
            if distance_error > 5.0:  # More than 5cm off
                base_speed *= 0.5  # Slow down significantly
            elif distance_error > 2.0:  # More than 2cm off  
                base_speed *= 0.7  # Slow down moderately
        
        # Calculate turning component - stronger control for curves
        turn_multiplier = 2.0 if target_wp.curvature > 0 else 1.0
        turn_speed = self.pid_heading.update(heading_error, 0.1) * turn_multiplier
    ...
    def simulate_robot_step_fixed(self, target_wp: WayPoint, dt: float) -> RobotState:
        """FIXED: Simulate one step with proper differential drive kinematics and North = 0°"""
        ...
        # Normalize heading to [0, 2π] for North = 0°
    ...
    def print_debug_comparison(self):
        """Print detailed comparison of target vs actual with North = 0°"""
        print("\n" + "="*100)
        print("TARGET vs ACTUAL COMPARISON (NORTH = 0°)")
        print("="*100)
    ...
    def create_comprehensive_plot(self):
        ...
        ax_main.set_title(f'FIXED Robot Path V13 (North=0°) - Command: {self.planner.current_command}')
        ...
        ax_commands.set_title('Command Analysis (North=0°)')
    ...
    def animate_robot_motion(self):
        ...
        print("=== ANIMATING FIXED ROBOT MOTION V13 (NORTH = 0°) ===")
...
# =============================================================================
# MAIN SIMULATION - NORTH = 0°
# =============================================================================
def main():
    test_commands = [
        "R",            # Simple right turn (North -> East)
       # //"RR",            # Simple left turn (North -> West)
       # "RRR",         # Complete circle (should return to origin)
       #
       #  "RRRR",          # Move forward, turn right, move forward
    ]
    
    print("Robot Path Planner V13 - FIXED with North = 0°")
